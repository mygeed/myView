...
const { parse, compileTemplate, TemplateCompiler } = require('@vue/compiler-sfc');
...
module.exports = function (source) {
  ...
  source = `[TOC]\n${source}`;
  let styleCodeList = [], demoBlockId = 0;
  // 需要解析成 vue 代码块集合
  const componentCodeList = [], md = mdIt({
      ...
      if (tokens[idx].nesting === 1) {
        ...
        if (script) {
          ...
          script = script.replace(/export\s+default/, 'const exportJavaScript =')
            ...
            .replace(/import ({.*}) from 'myView'/g, (s, s1) => `const ${s1} = require('myView')`);
        } else {
